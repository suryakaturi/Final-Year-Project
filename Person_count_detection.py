...
Expected = int(input('Please enter the expected no of people: '))
...
#t_minus = im.read("C:\Users\surya\Downloads\download")
        ...
        while(cap.isOpened()):
            ...
            if image is not None and image2 is not None and ret is True and ret2 is True:
                ...
                if cv2.countNonZero(diffImg(t_minus, t, t_plus)) > threshold and timeCheck != datetime.now().strftime(
                    ...
                    df1 = pd.DataFrame(boxes.reshape(100, 4), columns=['y_min', 'x_min', 'y_max', 'x_max'])
                    df2 = pd.DataFrame(classes.reshape(100, 1), columns=['classes'])
                    df3 = pd.DataFrame(scores.reshape(100, 1), columns=['scores'])
                    df4 = pd.concat([df, df1, df2], axis=1)
                    df5 = df4.loc[df4['classes'] == 1]
                    df6 = df5.loc[df5['scores'] > 0.50]

                    if int(len(df6.index)) > 0:
                        people = int(len(df6.index))
                    else:
                        people = 0
                    volume = 0
                    
                    v_list = ['150 db', '180 db', '200 db', '250 db', '300 db', '350 db', '400 db']
                    if 1>=people<=1:
                        volume = v_list[0]
                    elif(people > 1 and people <= 5):
                        volume = v_list[1]
                    elif(people > 5 and people <=  10):
                        volume = v_list[2]
                    elif(people > 10 and people <= 15):
                        volume = v_list[3]
                    elif (people >15 and people <= 20):
                        volume = v_list[4]
                    elif (people >20 and people <= 25):
                        volume = v_list[5]
                    elif (people >25 and people <= 30):
                        volume = v_list[6]
                    #print(volume , "Hz")
                    #columns = ['LocationID','DeviceID','DeviceTime', 'Class', 'Count','Volume Required', 'People Expected','Accuracy']
                    columns = ['DeviceTime','Count','Volume Required', 'People Expected','Accuracy']
                    
                    index = [0]
                    Accuracy = str(("{:.2f}".format(float(people)/float(Expected) * 100))) + ' %'
                    ...
                    df_.loc[0, 'Volume Required'] = volume
                    df_.loc[0, 'People Expected'] = Expected
                    df_.loc[0, 'Accuracy'] = Accuracy
